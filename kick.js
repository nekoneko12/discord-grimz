...
module.exports = {
    name: 'kick',
    description: 'Menendang member dari server.',
    ...
    async execute(message, args) {
        ...
        if (member.id === message.author.id) {
            return message.reply({
                embeds: [new EmbedBuilder().setColor('#992D22').setDescription('❌ Anda tidak bisa menendang diri sendiri.')]
            });
        }

        if (!member.kickable) {
            return message.reply({
                embeds: [new EmbedBuilder().setColor('#992D22').setDescription('❌ Saya tidak bisa menendang member ini. Mungkin perannya lebih tinggi dari saya.')]
            });
        }
